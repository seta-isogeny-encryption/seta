...
GEN norm_equation_special(GEN p, GEN M, long parity, bool randomized) {
    pari_sp ltop = avma;
    GEN upper_bound_pari = gdivent(M,p); // gdivent is the euclidean division
...
// solve x^2 + q*y^2 + p(u^2 + q*v^2) = M, with (u,v) != (0,0)
GEN norm_equation_special_q(GEN p, long q, GEN M) {
    ...
    long bound_randomized_v = (long)(sqrt(bound_randomized));

    for (int i = 0; i < bound_randomized*10; ++i) {
        u = random_Fl(bound_randomized);
        v = random_Fl(bound_randomized_v);
