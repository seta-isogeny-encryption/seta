...
void poly_mul(fp2 *c,const fp2 *a,long long alen,const fp2 *b,long long blen)
{
  ...
  if (alen == 2) {
    fp2_mul3(&c[0],&a[0],&b[0]);
    fp2_mul3(&c[2],&a[1],&b[1]);

    fp2 a01; fp2_add3(&a01,&a[0],&a[1]);
    fp2 b01; fp2_add3(&b01,&b[0],&b[1]);
    fp2_mul3(&c[1],&a01,&b01);
    fp2_sub2(&c[1],&c[0]);
    fp2_sub2(&c[1],&c[2]);

    /*
    fp2_mul3(&c[1],&a[0],&b[1]);
    fp2 t;
    fp2_mul3(&t,&a[1],&b[0]);
    fp2_add2(&c[1],&t);
    */
  ...
  if (blen == 2) {
    if (alen == 3) {
      ...
      fp2_sub2(&c[1],&c[0]);
      fp2_sub2(&c[1],&c[2]);
      fp2_mul3(&c[3],&a[2],&b[1]);
      fp2 a2b0; fp2_mul3(&a2b0,&a[2],&b[0]);
      fp2_add2(&c[2],&a2b0);
      return;
    }
    if (alen == 4) {
      ...
      fp2 mid;
      fp2_mul3(&mid,&a[2],&b[0]);
      fp2_mul3(&c[4],&a[3],&b[1]);
      fp2 a23; fp2_add3(&a23,&a[2],&a[3]);
      fp2_mul3(&c[3],&a23,&b01);
      fp2_sub2(&c[3],&mid);
      fp2_sub2(&c[3],&c[4]);
      fp2_add2(&c[2],&mid);
...
/*
      ...
      fp2_add2(&c[2],&a2b0);
      fp2_mul3(&c[4],&a[3],&b[1]);
      fp2 a3b0; fp2_mul3(&a3b0,&a[3],&b[0]);
      fp2_add2(&c[3],&a3b0);
*/
      return;
    }
  ...
  if (blen == 3) {
    if (alen <= 3) {
      ...
      /* XXX: toom instead? */
      fp2 a10; fp2_sub3(&a10,&a[1],&a[0]);
      fp2 b01; fp2_sub3(&b01,&b[0],&b[1]);
      fp2_mul3(&c[1],&a10,&b01);
      fp2 a20; fp2_sub3(&a20,&a[2],&a[0]);
      fp2 b02; fp2_sub3(&b02,&b[0],&b[2]);
      fp2_mul3(&c[2],&a20,&b02);
      fp2 a21; fp2_sub3(&a21,&a[2],&a[1]);
      fp2 b12; fp2_sub3(&b12,&b[1],&b[2]);
      fp2_mul3(&c[3],&a21,&b12);
      fp2_mul3(&c[0],&a[0],&b[0]);
      fp2_mul3(&c[4],&a[2],&b[2]);
      fp2 a1b1; fp2_mul3(&a1b1,&a[1],&b[1]);
      fp2 t; fp2_add3(&t,&a1b1,&c[0]);
      fp2_add2(&c[1],&t);
      fp2_add3(&t,&a1b1,&c[4]);
      fp2_add2(&c[3],&t);
      fp2_add2(&t,&c[0]);
      ...
      if (alen >= 4) {
        fp2_mul3(&t,&a[3],&b[0]); fp2_add2(&c[3],&t);
        fp2_mul3(&t,&a[3],&b[1]); fp2_add2(&c[4],&t);
        fp2_mul3(&c[5],&a[3],&b[2]);
      }
      if (alen >= 5) {
        ...
        // a4b2
        fp2_mul3(&t,&a[4],&b[0]); fp2_add2(&c[4],&t);
        fp2_mul3(&t,&a[4],&b[1]); fp2_add2(&c[5],&t);
        fp2_mul3(&c[6],&a[4],&b[2]);
      }
      return;
    }
  }
  ...
  fp2 *a01 = malloc(sizeof(fp2)*kara);
  fp2 *b01 = malloc(sizeof(fp2)*kara);

  for (long long i = 0;i < a1len;++i)
    fp2_add3(&a01[i],&a[i],&a[i+kara]);
  for (long long i = a1len;i < kara;++i)
    a01[i] = a[i];

  for (long long i = 0;i < b1len;++i)
    fp2_add3(&b01[i],&b[i],&b[i+kara]);
  for (long long i = b1len;i < kara;++i)
    b01[i] = b[i];

  fp2 *c01 = malloc(sizeof(fp2)*(kara+kara-1));
  long long c1len = a1len+b1len-1;
  fp2 *c1 = malloc(sizeof(fp2)*c1len);
  ...
  if (c1len < kara) {
    fp2_sub3(&c[kara+kara-1],&c01[kara-1],&c[kara-1]);
    for (long long i = 0;i < c1len;++i) {
      fp2_sub3(&mix,&c[kara+i],&c1[i]);
      fp2_sub3(&c[i+2*kara],&c01[i+kara],&mix);
      fp2_sub3(&c[i+kara],&mix,&c[i]);
      fp2_add2(&c[i+kara],&c01[i]);
    }
    for (long long i = c1len;i < kara-1;++i) {
      fp2_sub3(&c[i+2*kara],&c01[i+kara],&c[i+kara]);
      fp2_sub2(&c[i+kara],&c[i]);
      fp2_add2(&c[i+kara],&c01[i]);
    }
    return;
  }
  ...
  fp2_sub3(&c[kara+kara-1],&c01[kara-1],&c[kara-1]);
  fp2_sub2(&c[kara+kara-1],&c1[kara-1]);
...
void poly_mul_low(fp2 *c,long long clen,const fp2 *a,long long alen,const fp2 *b,long long blen)
{
  ...
  if (clen*4 >= 3*(alen+blen-1)) { /* XXX: tune cutoff */
    fp2 *ab = malloc(sizeof(fp2)*(alen+blen-1));
  ...
  if (alen < blen) {
    const fp2 *t = a; long long tlen = alen;
    a = b; alen = blen;
    b = t; blen = tlen;
  }
  ...
  if(1) { /* XXX: tune this */
    ...
    fp2 *a0 = malloc(sizeof(fp2)*a0len);
    fp2 *a1 = malloc(sizeof(fp2)*a1len);
    ...
    for (long long i = 0;i < c1len-1;++i) fp2_add2(&c[2*i+2],&c1[i]);
    if (2*c1len < clen) fp2_add2(&c[2*c1len],&c1[c1len-1]);

    free(a0); free(a1); free(a01);
    free(b0); free(b1); free(b01);
    free(c0); free(c1); free(c01);
    return;
  }
...
void poly_mul_selfreciprocal(fp2 *c,const fp2 *a,long long alen,const fp2 *b,long long blen)
{
  ...
  if (alen == 4 && blen == 4) {
    fp2_mul3(&c[0],&a[0],&b[0]);
    fp2_mul3(&c[3],&a[1],&b[1]);
    fp2 a01; fp2_add3(&a01,&a[0],&a[1]);
    fp2 b01; fp2_add3(&b01,&b[0],&b[1]);
    fp2_mul3(&c[2],&a01,&b01);
    fp2_sub2(&c[2],&c[0]);
    fp2_sub3(&c[1],&c[2],&c[3]);
    fp2_add2(&c[3],&c[0]);
    fp2_add2(&c[3],&c[3]);
    c[4] = c[2];
    c[5] = c[1];
  ...
  if (alen == 5 && blen == 5) {
    ...
    fp2_mul3(&c[0],&a[0],&b[0]);
    fp2 a1b1; fp2_mul3(&a1b1,&a[1],&b[1]);
    fp2 a2b2; fp2_mul3(&a2b2,&a[2],&b[2]);
  ...
  if (alen == blen && (alen&1)) {
    ...
    fp2 *a0 = malloc(sizeof(fp2)*(len0));
    fp2 *b0 = malloc(sizeof(fp2)*(len0));
    fp2 *a1 = malloc(sizeof(fp2)*(len1));
    fp2 *b1 = malloc(sizeof(fp2)*(len1));
    fp2 *c0 = malloc(sizeof(fp2)*(len0+len0-1));
    fp2 *c01 = malloc(sizeof(fp2)*(len0+len0-1));
    fp2 *c1 = malloc(sizeof(fp2)*(len1+len1-1));
    ...
    for (long long i = 0;i < len1;++i) fp2_add2(&a0[i],&a1[i]);
    for (long long i = 0;i < len1;++i) fp2_add2(&b0[i],&b1[i]);
    for (long long i = 0;i < len1;++i) fp2_add2(&a0[i+1],&a1[i]);
    for (long long i = 0;i < len1;++i) fp2_add2(&b0[i+1],&b1[i]);
    ...
    for (long long i = 0;i < len1+len1-1;++i)
      fp2_sub2(&c01[i],&c1[i]);
    for (long long i = 0;i < len1+len1-1;++i)
      fp2_sub2(&c01[i+1],&c1[i]);
    for (long long i = 0;i < len1+len1-1;++i)
      fp2_sub2(&c01[i+1],&c1[i]);
    for (long long i = 0;i < len1+len1-1;++i)
      fp2_sub2(&c01[i+2],&c1[i]);

    for (long long i = 1;i < len0+len0-1;++i)
      fp2_sub2(&c01[i],&c01[i-1]);
    ...
    for (long long i = 0;i < len1+len1-1;++i) fp2_add2(&c[2*i+2],&c1[i]);
  ...
  if (alen == blen && !(alen&1)) {
    ...
    fp2 *c0 = malloc(sizeof(fp2)*(alen-1));
    fp2 *c1 = malloc(sizeof(fp2)*(alen-1));

    poly_mul(c0,a,half,b,half);
    poly_mul(c1,a,half,b+half,half);

    for (long long i = 0;i < alen+alen-1;++i) c[i] = fp2_0;
    for (long long i = 0;i < alen-1;++i)
      fp2_add2(&c[i],&c0[i]);
    for (long long i = 0;i < alen-1;++i)
      fp2_add2(&c[alen+alen-2-i],&c0[i]);
    for (long long i = 0;i < alen-1;++i)
      fp2_add2(&c[half+i],&c1[i]);
    for (long long i = 0;i < alen-1;++i)
      fp2_add2(&c[alen+half-2-i],&c1[i]);
...
void poly_mul_high(fp2 *c,long long cstart,const fp2 *a,long long alen,const fp2 *b,long long blen)
{
  ...
  if (cstart == alen+blen-2) {
    fp2_mul3(&c[0],&a[alen-1],&b[blen-1]);
    return;
  }
  if (blen == 1) {
    for (long long i = cstart;i < alen+blen-1;++i)
      fp2_mul3(&c[i-cstart],&a[i],&b[0]);
    return;
  }
  if (cstart == alen+blen-3) {
    fp2_mul3(&c[0],&a[alen-2],&b[blen-1]);
    fp2 t;
    fp2_mul3(&t,&a[alen-1],&b[blen-2]);
    fp2_add2(&c[0],&t);
    fp2_mul3(&c[1],&a[alen-1],&b[blen-1]);
    return;
  }

  fp2 *arev = malloc(sizeof(fp2)*(alen));
  fp2 *brev = malloc(sizeof(fp2)*(blen));
  for (long long i = 0;i < alen;++i)
    arev[alen-1-i] = a[i];
  for (long long i = 0;i < blen;++i)
    brev[blen-1-i] = b[i];

  fp2 *crev = malloc(sizeof(fp2)*(alen+blen-1-cstart));
  poly_mul_low(crev,alen+blen-1-cstart,arev,alen,brev,blen);
  for (long long i = cstart;i < alen+blen-1;++i)
    c[i-cstart] = crev[alen+blen-2-i];

  free(arev); free(brev); free(crev);
/*
  fp2 ab[alen+blen-1];
  poly_mul(ab,a,alen,b,blen);
  for (long long i = cstart;i < alen+blen-1;++i)
...
void poly_mul_mid(fp2 *c,long long cstart,long long clen,const fp2 *a,long long alen,const fp2 *b,long long blen)
{
  ...
  if (clen == 2) {
    ...
    if (cstart == 1 && alen >= 3 && blen == 2) {
      ...
      fp2 delta;
      fp2_sub3(&delta,&b[0],&b[1]);
      fp2_mul2(&delta,&a[1]);
      fp2_add3(&c[0],&a[0],&a[1]);
      fp2_mul2(&c[0],&b[1]);
      fp2_add2(&c[0],&delta);
      fp2_add3(&c[1],&a[1],&a[2]);
      fp2_mul2(&c[1],&b[0]);
      fp2_sub2(&c[1],&delta);
      return;
    }
    if (cstart == 3 && alen >= 5 && blen == 4) {
      ...
      fp2 b01; fp2_sub3(&b01,&b[0],&b[1]);
      fp2 b23; fp2_sub3(&b23,&b[2],&b[3]);
      fp2 a1b23; fp2_mul3(&a1b23,&a[1],&b23);
      fp2 a3b01; fp2_mul3(&a3b01,&a[3],&b01);
      fp2 delta; fp2_add3(&delta,&a1b23,&a3b01);
      fp2 a01; fp2_add3(&a01,&a[0],&a[1]);
      fp2 a12; fp2_add3(&a12,&a[1],&a[2]);
      fp2 a23; fp2_add3(&a23,&a[2],&a[3]);
      fp2 a34; fp2_add3(&a34,&a[3],&a[4]);
      fp2_mul2(&a01,&b[3]);
      fp2_mul2(&a12,&b[2]);
      fp2_mul2(&a23,&b[1]);
      fp2_mul2(&a34,&b[0]);
      fp2_add3(&c[0],&a01,&a23);
      fp2_add3(&c[1],&a12,&a34);
      fp2_add2(&c[0],&delta);
      fp2_sub2(&c[1],&delta);
      return;
    }
  ...
  if (clen == 3 && cstart == 1 && blen == 2 && alen >= 4) {
    ...
    // c[2] = a[2]*b[1]+a[3]*b[0]
    fp2 b01;
    fp2_sub3(&b01,&b[0],&b[1]);
    fp2 delta0;
    fp2_mul3(&delta0,&a[1],&b01);
    fp2 delta1;
    fp2_mul3(&delta1,&a[2],&b01);
    fp2_add3(&c[0],&a[0],&a[1]);
    fp2_add3(&c[1],&a[1],&a[2]);
    fp2_add3(&c[2],&a[2],&a[3]);
    fp2_mul2(&c[0],&b[1]);
    fp2_mul2(&c[1],&b[1]);
    fp2_mul2(&c[2],&b[0]);
    fp2_add2(&c[0],&delta0);
  ...
  if (clen == 4 && cstart == 5 && blen == 6 && alen >= 9) {
    ...
    fp2 a01[6];
    for (long long i = 0;i < 6;++i) fp2_add3(&a01[i],&a[i],&a[i+3]);

    fp2 b01[3];
    for (long long i = 0;i < 3;++i) fp2_sub3(&b01[i],&b[i],&b[i+3]);
    ...
    fp2_add2(&c[0],&delta[0]);
    fp2_add2(&c[1],&delta[1]);
    fp2_add2(&c[2],&delta[2]);
    fp2_sub2(&c[3],&delta[0]);
    return;
  }
  ...
  if (clen == 5 && cstart == 3 && blen == 4 && alen >= 8) {
    ...
    fp2 a01[6];
    fp2_add3(&a01[0],&a[0],&a[2]);
    fp2_add3(&a01[1],&a[1],&a[3]);
    fp2_add3(&a01[2],&a[2],&a[4]);
    fp2_add3(&a01[3],&a[3],&a[5]);
    fp2_add3(&a01[4],&a[4],&a[6]);
    fp2_add3(&a01[5],&a[5],&a[7]);

    fp2 b01[2];
    fp2_sub3(&b01[0],&b[0],&b[2]);
    fp2_sub3(&b01[1],&b[1],&b[3]);
    ...
    fp2_add2(&c[1],&delta[1]);
    fp2_add2(&c[2],&delta[2]);
    fp2_sub2(&c[3],&delta[1]);
    fp2_sub2(&c[4],&delta[2]);
    return;
  }
  ...
  if ((clen&1) && cstart == clen && blen == clen+1 && alen >= cstart+clen) {
    ...
    fp2 *a01 = malloc(sizeof(fp2)*(3*split-2));
    assert(3*split-2+split <= alen);
    for (long long i = 0;i < 3*split-2;++i) fp2_add3(&a01[i],&a[i],&a[i+split]);

    fp2 *b01 = malloc(sizeof(fp2)*(split));
    assert(2*split == blen);
    for (long long i = 0;i < split;++i) fp2_sub3(&b01[i],&b[i],&b[i+split]);
    ...
    fp2 *delta = malloc(sizeof(fp2)*(split));
    poly_mul_mid(delta,split-1,split,a+split,clen,b01,split);

    for (long long i = 0;i < split;++i) fp2_add2(&c[i],&delta[i]);
    for (long long i = 0;i < split-1;++i) fp2_sub2(&c[i+split],&delta[i]);
    free(a01); free(b01); free(delta);
    return;
  }
  ...
  if (!(clen&1) && cstart == clen-1 && blen == clen && alen >= cstart+clen) {
    ...
    fp2 *a01 = malloc(sizeof(fp2)*(3*split-1));
    for (long long i = 0;i < 3*split-1;++i) fp2_add3(&a01[i],&a[i],&a[i+split]);
    ...
    for (long long i = 0;i < split;++i) {
      fp2_add2(&c[i],&delta[i]);
      fp2_sub2(&c[split+i],&delta[i]);
    }
    free(a01); free(b01); free(delta);
    return;
  }
  ...
  if (cstart+cstart+clen < alen+blen) {
    fp2 *ab = malloc(sizeof(fp2)*(cstart+clen));
  ...
  fp2 *ab = malloc(sizeof(fp2)*(alen+blen-1-cstart));
...
void poly_eval_precompute(fp2 *precomp,long long flen,const proj *p)
{
  if (flen <= 2) return;

  fp2 *pxpow = malloc(sizeof(fp2)*(flen));
  fp2 *pzpow = malloc(sizeof(fp2)*(flen));

  pxpow[1] = p->x;
  pzpow[1] = p->z;
  for (long long i = 2;i < flen;++i) {
    fp2_mul3(&pxpow[i],&pxpow[i-1],&p->x);
    fp2_mul3(&pzpow[i],&pzpow[i-1],&p->z);
  }

  precomp[0] = pzpow[flen-1];
  precomp[flen-1] = pxpow[flen-1];
  for (long long i = 1;i < flen-1;++i)
    fp2_mul3(&precomp[i],&pxpow[i],&pzpow[flen-1-i]);

  free(pxpow); free(pzpow);
}
...
void poly_eval_postcompute(fp2 *v,const fp2 *f,long long flen,const proj *p,const fp2 *precomp)
{
  assert(flen > 0);
  if (flen == 1) {
    *v = f[0];
  ...
  if (flen == 2) {
    fp2 tmp;
    fp2_mul3(v,&f[0],&p->z);
    fp2_mul3(&tmp,&f[1],&p->x);
    fp2_add2(v,&tmp);
    return;
  }

  fp2_mul3(v,&f[0],&precomp[0]);

  for (long long i = 1;i < flen;++i) {
    fp2 tmp;
    fp2_mul3(&tmp,&f[i],&precomp[i]);
    fp2_add2(v,&tmp);
  }
}

void poly_eval(fp2 *v,const fp2 *f,long long flen,const proj *p)
{
...
void poly_pseudoreciprocal(fp2 *d,fp2 *r,long long rlen,const fp2 *m,long long mdeg)
{
  if (mdeg == 0) {
    r[rlen-1] = fp2_1;
    for (long long i = 0;i < rlen-1;++i) r[i] = fp2_0;
    *d = m[mdeg];
    return;
  }
  if (rlen == 1) {
  ...
  fp2 *eps = malloc(sizeof(fp2)*(mdeg));
...
void poly_pseudoremainder_precompute(fp2 *precomp,long long glen,long long flen,const fp2 *m)
{
  assert(flen >= glen);
  if (flen == glen) return;
  long long vlen = flen-glen;
  fp2 *d = precomp; /* length 1 */
  fp2 *v = precomp+1; /* length vlen */
  poly_pseudoreciprocal(d,v,vlen,m,glen);
}

void poly_pseudoremainder_postcompute(fp2 *g,long long glen,const fp2 *f,long long flen,const fp2 *m,const fp2 *precomp)
{
  assert(flen >= glen);
  ...
  long long vlen = flen-glen;
  const fp2 *d = precomp; /* length 1 */
  const fp2 *v = precomp+1; /* length vlen */
  ...
  fp2 *vf = malloc(sizeof(fp2)*(vlen));
  poly_mul_high(vf,flen-1,v,vlen,f,flen);

  fp2 *qm = malloc(sizeof(fp2)*(glen));
...
void poly_pseudoremainder(fp2 *g,long long glen,const fp2 *f,long long flen,const fp2 *m)
{
...
void poly_multieval_unscaled_precompute(fp2 *precomp,long long n,long long flen,const fp2 *P,const fp2 *T)
{
  if (n <= 0) return;
...
void poly_multieval_unscaled_postcompute(fp2 *v,long long n,const fp2 *f,long long flen,const fp2 *P,const fp2 *T,const fp2 *precomp)
{
  if (n <= 0) return;
...
void poly_multieval_unscaled(fp2 *v,long long n,const fp2 *f,long long flen,const fp2 *P,const fp2 *T)
{
...
void poly_multieval_scaled(fp2 *v,long long n,const fp2 *r,const fp2 *P,const fp2 *T)
{
  if (n <= 0) return;
  if (n == 1) {
    v[0] = r[0];
