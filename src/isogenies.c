...
//Internal function
//supersedes eval_rtl_old using the recursive function for better isogeny computation
...
void dual(proj *A, odd_isogeny *phi) {
  ...
  // The p+1 part
  Q[2] = phi->kernel_minus;
  dual_one_side(A, &phi->kernel_plus, curve_order_is_p_plus_one, phi->deg_plus,
		Q, 3, &p_plus_odd_cofactor, p_plus_fact, p_plus_mult, p_plus_len);

  // The p-1 part
  phi->kernel_minus = Q[2];
  Q[2] = Q[0];
  dual_one_side(A, &phi->kernel_minus, !curve_order_is_p_plus_one, phi->deg_minus,
